...
def test_success_writes_lua_script_into_scripts_edit_dir(monkeypatch, fake_video, isolated_dirs, capsys):
    ...
    assert lua_files[0].name == "Auto Template - my_video.lua"
...
def test_second_analysis_of_same_video_gets_a_version_suffix(monkeypatch, fake_video, isolated_dirs, capsys):
    projects_root, scripts_dir = isolated_dirs
    ...
    assert build_project.main([str(fake_video)]) == 0
    assert build_project.main([str(fake_video)]) == 0
    assert build_project.main([str(fake_video)]) == 0

    lua_names = sorted(p.name for p in scripts_dir.glob("*.lua"))
    assert lua_names == [
        "Auto Template - my_video.lua",
        "Auto Template - my_video_2.lua",
        "Auto Template - my_video_3.lua",
    ]
    project_names = sorted(p.name for p in projects_root.iterdir())
    assert project_names == ["my_video", "my_video_2", "my_video_3"]
